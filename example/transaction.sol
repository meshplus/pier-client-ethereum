...
contract Transaction {
    ...
    function register(address addr, bool ordered, uint admins, uint64 _adminThreshold) public onlyBroker {
    ...
    function audit(address addr, int64 status, address voter, uint admins) public onlyBroker returns (bool) {
    ...
    function getLocalServiceList(string memory bitxhubID, string memory appchainID) public view returns (string[] memory) {
    ...
    function genFullServiceID(string memory bitxhubID, string memory appchainID, string memory serviceID) private view returns (string memory) {
